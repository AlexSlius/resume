...
export const fetchPostAddCvHobie = createAsyncThunk('countrus/fetchPostAddCvHobie', async ({ idCv, data }, thunkAPI) => {
    ...
    await thunkAPI.dispatch(fetchGetCvHobie({ idCv, isDrawing: true }));
    return response;
});

export const fetchGetCvHobie = createAsyncThunk('countrus/fetchGetCvHobie', async ({ idCv, isDrawing = false }, thunkAPI) => {
