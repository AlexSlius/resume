                ...
                <div className="links-list">
                  {
                    isArray(social_links) && social_links.length && (
                      social_links.map((item, index) => (
                        <a href={item.link} className="links-item additional-color-1-border m-border additional-color-1-text" key={index}>
                      ...
                      <div className="block-info" key={index}>
                        <div className={`cv-subheading font-size-2 ${!itemEd.degree && !itemEd.study && !itemEd.facility && !beforeСontent ? 'hide' : ''}`}>
                          ...
                          <span className={`${!itemEd.degree ? 'empty-field' : ''} ${!itemEd.facility && !beforeСontent ? 'hide' : ''}`}>
                            {checkForSymbol([itemEd?.facility]) ? (itemEd?.facility || ', Harcum College, Portland') + ', ' : itemEd?.facility || ', Harcum College, Portland'}
                          </span>
